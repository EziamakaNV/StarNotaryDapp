...
it('can change owners', async () => {
 ...
 it('can change the name of the star', async() => {
     let instance = await StarNotary.deployed();
     let originalName = await instance.starName.call();
     assert.equal(originalName,"Awesome Udacity Star");
     await instance.changeName("Helios", {from: owner});
     let newName = await instance.starName.call();
     assert.equal(newName, "Helios");
 });